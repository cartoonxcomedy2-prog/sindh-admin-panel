        ...
        <div id="sch-programs-bachelor" className="form-section" >
          ...
          <div className="programs-list">
            {bachelorPrograms.map((prog, idx) => (
              <div className="program-card" key={`sbach-${idx}`}>
                <div className="program-card-header">
        ...
        <div id="sch-programs-master" className="form-section" >
          ...
          <div className="programs-list">
            {masterPrograms.map((prog, idx) => (
              <div className="program-card" key={`smast-${idx}`}>
                <div className="program-card-header">
        ...
        <div id="sch-universities" className="form-section" >
          ...
          {allUniversities.length === 0 ? (
            ...
            const filtered = allUniversities.filter(u => {
              const matchesSearch = u.name.toLowerCase().includes(searchTermUni.toLowerCase());
              ...
              <>
                <div style={{ display: 'flex', gap: 10, marginBottom: 15 }}>
                  ...
                  <button 
                    ...
                    className="btn-save-draft" 
                    style={{ padding: '6px 12px', fontSize: 11 }}
                    onClick={() => deselectAllFiltered(filtered)}
                  >
                ...
                <div style={{ 
                  ...
                  {filtered.length === 0 ? (
                    ...
                    filtered.map((uni) => (
                      <div
                        key={uni._id}
                        className={`uni-selection-item ${linkedUniversities.includes(uni._id) ? 'selected' : ''}`}
                        onClick={() => toggleUniversity(uni._id)}
                        style={{ 
                          display: 'flex', 
                          alignItems: 'center', 
                          gap: 12,
                          ...
                          transition: 'all 0.2s',
                          marginBottom: '4px',
                          background: linkedUniversities.includes(uni._id) ? 'rgba(76, 201, 240, 0.1)' : 'transparent',
                          border: `1px solid ${linkedUniversities.includes(uni._id) ? '#4cc9f0' : 'transparent'}`
                        }}
                      >
                        <input 
                          type="checkbox" 
                          checked={linkedUniversities.includes(uni._id)} 
                          readOnly 
                          style={{ width: 18, height: 18, cursor: 'pointer' }}
                        />
                        {uni.thumbnail ? (
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: '600', fontSize: 14 }}>{uni.name}</div>
                          <div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>{uni.city}, {uni.country}</div>
                        </div>
                        {linkedUniversities.includes(uni._id) && <span style={{ color: '#4361ee', fontWeight: 'bold' }}>✓</span>}
                      </div>
                    ))
                  )}
                </div>
