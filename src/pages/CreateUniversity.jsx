...
const ProgramItem = memo(({ prog, idx, group, onUpdate, onRemove, programOptions, durations, feeStructures, currency }) => {
  return (
    <div className="program-card">
      <div className="program-card-header">
        <h4>{group} Program #{idx + 1}</h4>
        {onRemove && <button type="button" className="btn-remove" onClick={() => onRemove(group, idx)}>×</button>}
      </div>
      ...
      <div className="form-row three" style={{ marginTop: 14 }}>
        ...
        <div className="form-group">
          ...
          <select value={prog.feeStructure} onChange={(e) => onUpdate(group, idx, 'feeStructure', e.target.value)}>
            {feeStructures.map((f) => <option key={f}>{f}</option>)}
          </select>
        </div>
        <div className="form-group">
          ...
          <input
            ...
            onChange={(e) => onUpdate(group, idx, 'feeAmount', e.target.value)}
...
export default function CreateUniversity() {
  ...
  useEffect(() => {
    if (!isEdit) {
      ...
      if (adminInfo.role === 'university') {
        API.get('/universities/admin/list').then(res => {
          const list = res.data.data || [];
          if (list.length > 0) {
            navigate(`/universities/edit/${list[0]._id}`, { replace: true });
          }
          ...
          <div className="programs-list">
            {bachelorPrograms.map((prog, idx) => (
              <ProgramItem
                ...
                onRemove={bachelorPrograms.length > 1 ? removeProgram : null}
          ...
          <div className="programs-list">
            {masterPrograms.map((prog, idx) => (
              <ProgramItem
                ...
                group="Master"
                onUpdate={updateProgram}
                onRemove={masterPrograms.length > 1 ? removeProgram : null}
